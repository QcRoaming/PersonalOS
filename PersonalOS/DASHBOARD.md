...
|---|---|---:|---|---|
| `research.kernel_aware_gemm` | main | P0 | active | 第三章事实与规则、第四章空间生成、第五章真实 BLIS direct/fringe/dynamic 以及 Chapter 4-5 集成均已有独立证据包。第六章 36-shape、B1-B4、五种子、25 回调预算的正式评估现已完成，包含优化动态 MLIR、完整 `bli_sgemm`、消融、探索壳、holdout、开销、置信区间和 11 幅论文图。Chapter 3 的 16 条源码核验记录已完成论文作者签核，当前主线转为按实测结果撰写第四至六章。 |
...
- research.kernel_aware_gemm: 当前只覆盖单线程、行主序 f32 与一个 Haswell 6x16 Contract；任意 stride、其他 dtype/微内核和跨主机泛化尚未验证。
