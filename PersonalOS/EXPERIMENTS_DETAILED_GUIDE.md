...
- **复现层级**：完整静态/重放分析运行 `python3 jlq/thesis/experiments/chapter6_contract_space_validation/scripts/run_all.py`；随后运行 `python3 -m unittest discover -s jlq/thesis/experiments/chapter6_contract_space_validation/tests -v`，当前 4/4 项通过。E4 不重新采集硬件计时，只重算冻结数据。
